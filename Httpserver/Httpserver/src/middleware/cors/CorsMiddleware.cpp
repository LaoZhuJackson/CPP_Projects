...
void CorsMiddleware::before(HttpRequest& request){
    ...
    if(rrequest.method() == HttpRequest::Method::kOptions){
...
void CorsMiddleware::handlePreflightRequest(const HttpRequest& request, HttpResponse& response){
    ...
    response.setStatusCode(HttpResponse::k204NoContent) //返回204 No Content（表示预检通过）
...
void CorsMiddleware::addCorsHeaders(HttpResponse& response, const std::string& origin){
    try{
        ...
        if(config_.allowCredentials) response.addHeader("Access-Control-Allow-Credentials","true");

        if(!config_.allowedMethods.empty()) response.addHeader("Access-Control-Allow-Methods",join(config_.allowedMethods, ", "));

        if(!config_.allowedHeaders.empty()) response.addHeader("Access-Control-Allow-Headers", join(config_.allowedHeaders, ", "));
