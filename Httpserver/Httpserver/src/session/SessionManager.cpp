...
SessionManager::SessionManager(std::unique_ptr<SessionStorage> storage) : storage_(std::move(storage)), rng_(std::random_device{}()){} // 初始化随机数生成器，用于生成随机的会话ID
...
std::shared_ptr<Session> SessionManager::getSession(const HttpRequest& req, HttpResponse* resp){
    ...
    if(!session || session->isExpired()){
        ...
        session = std::make_shared<Session>(sessinId,this);
        setSessionCookie(sessionId,resp);
    }else session->setManager(this); //为现有会话设置管理器
...
std::string SessionManager::generateSessionId(){
    std::stringstream ss; //stringstream 内部会预分配缓冲区，减少多次重新分配
    std::uniform_int_distribution<> dist(0,15)//均匀分布下生成的随机整数,默认类型int

    // 生成32个字符的会话ID，每个字符是一个十六进制数字
    for(int i=0;i<32;++i) ss<<std::hex<<dist(rng_)
...
std::string SessionManager::getSessionIdFromCookie(const HttpRequest& req){
    ...
    if(!cookie.empty()){
        size_t pos = cookie.find("sessionId=");//size_t:无符号整型
        if (pos != std::string::npos){
            pos += 10; // 跳过"sessionId="
            size_t end = cookie.find(';',pos);
            if(end!=std::string::npos) sessionId = cookie.substr(pos,end-pos);
            else sessionId = cookie.substr(pos);
        }
    }
    return sessionId;
}
