...
namespace http::db{
void DbConnectionPool::init(const std::string& host, const std::string& user,const std::string& password, const std::string& database, size_t poolSize){
...
std::shared_ptr<DbConnection> DbConnectionPool::getConnection(){
    std::shared_ptr<DbConnection> conn;
    {
        ...
        while(connections_.empty()){
            ...
            cv_.wait(lock);//线程会一直阻塞，直到：其他线程调用 cv_.notify_one() 或 cv_.notify_all()
        }
    ...
    try{
        ...
        return std::shared_ptr<DbConnection>(
            conn.get(),  // 原始指针
            [this,conn](DbConnection*){  // lambda自定义删除器
                std::lock_guard<std::mutex> lock(mutex_);
...
std::shared_ptr<DbConnection> DbConnectionPool::createConnection(){
    // 内存布局对比：
    // new + shared_ptr: [对象] [控制块] (两次分配)
    // make_shared:      [对象+控制块]    (一次分配)
