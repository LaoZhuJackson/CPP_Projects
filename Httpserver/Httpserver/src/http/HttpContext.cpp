...
bool HttpContext::parseRequest(Buffer* buf, Timestamp receiveTime){
    ...
    while(hasMore){
        if(state_ == kExpectRequestLine){
            ...
            if(crlf){
                ok = processRequestLine(buf->peek(), crlf);// buf->peek()返回缓冲区中可读数据的起始指针
                if(ok){
            ...
            const char* crlf = buf->find(buf->peek(),crlf,':');
            if(colon<crlf){
                rrequest_.addHeader(buf->peek(),colon,crlf);
                ...
                if(request_.method() == HttpRequest::kPost || request_.method() == HttpRequest::kPut){
                    ...
                    if(!contentLength.empty()){
                        ...
                        if(request_.contentLength() > 0) state_ = kExpectBody;
                        else{
                            state_ = kGotAll;
                            hasMore = false;
                        }
                    }else{
                        // POST/PUT 请求没有 Content-Length，是HTTP语法错误,POST/PUT请求必须包含Content-Length
                        ok = false;
                        hasMore = false;
                    }
                }
