...
#include <memory>
#include <functional>
#include <regex> //正则表达式库，提供了强大的模式匹配和文本处理能力
#include <vector>
...
public:
    ...
    struct RouteKeyHash{
        size_t operator()(const RouteKey& key) const{
            size_t methodHash = std::hash<int>{}(static_cast<int>(key.method));//使用了 C++ 的哈希函数来生成枚举值的哈希值
...
private:
    std::regex convertToRegex(const std::string& pathPattern){// 将路径模式转换为正则表达式，支持匹配任意路径参数
        std::string regexPattern = "^" + std::regex_replace(pathPattern, std::regex(R"(/:([^/]+))"), R"(/([^/]+))") + "$";//将路由模式映射成正则模式
    ...
    void extractPathParameters(const std::smatch& match,HttpRequest& request){
        ...
        for(size_t i = 1;i<match.size();++i) request.setPathParameterrs("param" + std::to_string(i), match[i].str());
    }
private:
    ...
    struct RouteHandlerObj
    {
        ...
        std::regex pathRegex_;
        HandlerPtr handler_;
        RouteHandlerObj(HttpRequest::Method method, std::regex pathRegex, HandlerPtr handler):method_(method),pathRegex_(pathRegex),HandlerPtr(handler){}
    };
