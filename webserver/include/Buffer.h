...
public:
    ...
    void retrieve(size_t len){
        if(len<readableBytes()) readerIndex_ += len; // 说明应用只读取了可读缓冲区数据的一部分，就是len长度 还剩下readerIndex+=len到writerIndex_的数据未读
        else retrieveAll();
    }
    void retrieveAll(){
    ...
    void ensureWritableBytes(size_t len){
        if(writableBytes()<len) makeSpace(len);//扩容
    }
...
private:
    ...
    void makeSpace(size_t len){
        ...
        | 8位  | 已读 | 待取 | 空位 |
         **/
        // 也就是说 len > xxx前面剩余的空间 + writer的部分
        if(writableBytes() + prependableBytes()<len + kCheapPrepend) buffer_.resize(writerIndex_+len);
        else{
