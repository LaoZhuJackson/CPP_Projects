#include "LogFile.h"
LogFile::LogFile(const std::string &basename,
                 off_t rollsize,
                 int flushInterval,
                 int checkEveryN ) : basename_(basename),
                                           rollsize_(rollsize),
                                           flushInterval_(flushInterval),
                                           checkEveryN_(checkEveryN),
                                           startOfPeriod_(0),
                                           lastRoll_(0),
                                           lastFlush_(0){
    // 重新启动时，可能没有log文件，因此在构建logFile对象，直接调用rollfile()创建一个新的log文件
    rollFile();
}
LogFile::~LogFile() = default;
void LogFile::append(const char* data,int len){
    std::lock_guard<std::mutex> lg(mutex_);//lock_guard保证构造时自动上锁，析构时自动解锁
    appendInlock(data,len);
}
void LogFile::flush(){
    file_->flush();
}
// 滚动日志
bool LogFile::rollFile(){
    time_t now = 0;
    std::string filename = getLogFileName(basename_,&now);
    time_t start = now / kRollPerSeconds_ * kRollPerSeconds_;//有取整操作：将任意时间戳对齐到最近的整点时间单位起点
    if(now > lastRoll_){
        lastFlush_ = now;
        lastRoll_ = now;
        startOfPeriod_ = start;

        // 创建新日志文件
        file_.reset(new FileUtil(filename));
        return true;
    }
    return false;
}
// 日志格式basename+now+.log"
std::string LogFile::getLogFileName(const std::string& basename,time_t* now){
    std::string filename;
    filename.reserve(basename.size() + 64);//预先分配足够的内存空间，避免多次重新分配
    filename = basename;

    char timebuf[32];
    struct tm tm;
    *now = time(NULL);//获取当前时间
    localtime_r(now,&tm);// 将time_t转换为本地时间的tm结构体
    strftime(timebuf,sizeof(timebuf),".%Y%m%d-%H%M%S", &tm);//时间格式化

    filename += timebuf;
    filename += ".log";
    return filename;
}

void LogFile::appendInlock(const char* data,int len){
    file_->append(data,len);
    time_t now = time(NULL);
    ++count_;

    // 1. 判断是否需要滚动日志
    if(file_->writtenBytes() > rollsize_){
        rollFile();
    }else if(count_ >= checkEveryN_)// 达到写入次数阈值后，进行检查
    {
        count_ = 0;
        // 基于时间周期滚动日志
        time_t thisPeriod = now / kRollPerSeconds_ * kRollPerSeconds_;
        if (thisPeriod != startOfPeriod_)//在这里进行了周期判断
        {
            rollFile();
        }
    }
    // 2. 判断是否需要刷新日志（独立的刷新逻辑）
    if (now - lastFlush_ > flushInterval_)
    {
        lastFlush_ = now;
        file_->flush();
    }
}